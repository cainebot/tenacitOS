...
// Phase 69 Plan 10 — update_identity_file_content approval payload renderer.
//
// Renders before/after content diff for a canonical agent_identity_files row.
// Data sources: payload.target_snapshot.prior_content (first 2000 chars) +
...
interface UpdateIdentityFileContentPayload {
  agent_id?: string;
  file_type?: string;
  content?: string;
...
export function ApprovalPayloadUpdateIdentityFileContent({
  payload,
}: {
  payload: UpdateIdentityFileContentPayload;
}) {
  ...
  return (
    <div
      data-testid="approval-payload-update_identity_file_content"
    ...
    >
      ...
      <section className="flex items-center gap-2">
        ...
        <span className="font-mono text-xs text-tertiary">
          {afterLen.toLocaleString()}/50,000 chars
        </span>
      </section>
      ...
      {afterLen > 2000 || before.length > 2000 ? (
        <div>
