...
export default function ActionsPage() {
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="mb-8">
        <h1
          className="text-3xl font-bold mb-2 font-[family-name:var(--font-display)] text-primary"
        >
          Quick Actions Hub
        </h1>
      ...
      {Object.keys(results).length > 0 && (
        <div className="rounded-xl bg-secondary border border-primary">
            ...
            {Object.values(results)
              ...
              .map((result) => {
                ...
                return (
                  <div
                    key={result.action}
                    className="flex items-center gap-3 p-3 cursor-pointer transition-colors hover:bg-tertiary"
                    onClick={() => setSelectedResult(result)}
                  >
                    <div style={{
                    ...
                    <Terminal className="w-3.5 h-3.5 flex-shrink-0 text-quaternary" />
                  </div>
                );
              })}
          </div>
      ...
      {confirmAction && (
        <div style={{
          ...
          <div className="bg-secondary rounded-2xl p-8 max-w-[400px] w-full border border-primary">
            <h3 className="text-primary mb-3 font-semibold">
              ⚠️ Confirm: {confirmAction.label}
            </h3>
            <p className="text-secondary mb-6 text-[0.9rem]">
              This action may affect running services. Are you sure?
            </p>
            <div className="flex gap-3 justify-end">
              <button
                onClick={() => setConfirmAction(null)}
                className="px-4 py-2 rounded-lg bg-tertiary text-secondary border-none cursor-pointer"
              >
                Cancel
              </button>
              <button
                onClick={() => executeAction(confirmAction)}
                className="px-4 py-2 rounded-lg bg-error-600 text-white border-none cursor-pointer font-semibold"
              >
                Run Anyway
              </button>
            </div>
