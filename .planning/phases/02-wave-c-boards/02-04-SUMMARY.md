...
key-decisions:
  - "ConfirmActionDialog imported from @circos/ui per migration spec (component needs to be added to package in future work)"
...
**Migrated the 921-line board detail page from 41 inline style={{}} objects and 54 var(--*) tokens to Tailwind UUI classes, removed @keyframes <style> block, and fixed ConfirmActionDialog import to @circos/ui — completing the Wave C boards migration.**
...
- Fixed `ConfirmActionDialog` import from `@/components/ui/confirm-action-dialog` to `@circos/ui`
- Added `cx` to the `@circos/ui` import for conditional class merging
...
- `ConfirmActionDialog` import moved to `@circos/ui` per migration spec; the component doesn't yet exist there (same pre-existing TS error as CardDetailPanel.tsx from plan 02-03) — this is a known gap to be addressed when `@circos/ui` package is expanded
...
## Issues Encountered
- Pre-existing TS error: `ConfirmActionDialog` not exported from `@circos/ui` — same issue exists in `CardDetailPanel.tsx` from plan 02-03. Fixed `onOpenChange` parameter type to `boolean` to eliminate the one new TS error our changes introduced; the `ConfirmActionDialog` export gap is a cross-plan concern.
...
- `ConfirmActionDialog` needs to be added to `@circos/ui` exports in a future plan to resolve the TS error across 2 files
